...
export const migrateWorld = async () => {
    ...
    if (worldSchemaVersion !== schemaVersion && game.user.isGM) {
        ...
        for (let pack of game.packs.filter((p: any) => p.metadata.package === "world" && ["Actor"].includes(p.metadata.entity))) {
...
const migrateActorData = (actor: Actor, worldSchemaVersion: number) => {
    const update: any = {};
...
const migrateItemData = (item: WrathAndGloryItem, worldSchemaVersion: number) => {
    const update: any = {};
    if (worldSchemaVersion < 1) {
        if (item.type === "weapon") {
            update["data.attack"] = {
...
const migrateSceneData = (scene: Scene, worldSchemaVersion: number) => {
    const tokens: Array<TokenData> = foundry.utils.deepClone(scene.tokens);
    return {
...
export const migrateCompendium = async function (pack: any, worldSchemaVersion: number) {
