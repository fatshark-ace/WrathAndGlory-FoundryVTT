...
export async function weaponRoll(rollData: RollData) {
...
export async function psychicRoll(rollData: RollData) {
...
export async function damageRoll(rollData: RollData) {
  _rollDamage(rollData);
  _computeDamageChat(rollData);
  await _sendDamageToChat(rollData);
}

export async function reroll(rollData: RollData) {
  rollData.rolls.hit = [];
...
function _rollPoolDice(rollData: RollData) {
...
function _rollDice(rollData: RollData, formula: string, isWrath: boolean) {
  ...
  r.terms.forEach((term) => {
    if (typeof term === 'object' && term !== null) {
      term.results.forEach(result => {
        ...
        rollData.result.dice.push(die as Die);
      });
    }
  });
  rollData.rolls.hit.push(r);
...
function _rollDamage(rollData: RollData) {
...
function _computeChat(rollData: RollData) {
...
function _computeDamageChat(rollData: RollData) {
...
function _computeDice(dieValue: number) {
  if (dieValue === 6) {
    return {
      name: "icon",
      value: 2,
...
function _computeExtraDice(dieValue: number, die: DieOptions) {
...
function _computeWrath(dieValue: number): Die {
  if (dieValue === 6) {
    return {
      name: "wrath-critical",
      value: 2,
...
function _countSuccess(rollData: RollData) {
  let success = 0;
...
function _countFailure(rollData: RollData) {
  let failure = 0;
  for (let i = 0; i < rollData.result.dice.length; i++) {
...
function _countShifting(rollData: RollData) {
  let shifting = 0;
...
function _hasWrathValue(rollData: RollData, dieValue: number) {
  for (let i = 0; i < rollData.result.dice.length; i++) {
...
async function _sendDamageToChat(rollData: RollData) {
